...
const addCustomer = function (name, email, image, userid, props, handleClose,tokenNo) {
  if (name == "" || email == "" || image == "") {
    Swal({
      title: "Error",
    ...
    storageRef.put(image).then(function (response) {
      response.ref.getDownloadURL().then(function (url) {
        ...
        firebase.firestore().collection("customer").add({
          name, email, image: url, userid,TokenNumber: tokenNo+1
        }).then(function () {
          ...
          firebase.firestore().collection("companies").doc(userid).get().then((res) => {
            ...
            firebase.firestore().collection("companies").doc(userid).update({
              ...
              Swal({
                title: "Token Bought",
                text: "you've successfully bought the token of this company",
                icon: "success",
              });
            }).catch(function (e) {
              alert(e.message)
...
const addCompany = function (compName, address, since, image, cid, latt, lngg, handleClose, currentToken,currentTokenBought) {
    ...
    storageRef.put(image).then(function (response) {
      response.ref.getDownloadURL().then(function (url) {
        ...
        firebase.firestore().collection("companies").add({
          compName, address, since, certificate: url, cid, latitude: latt, longitude: lngg, currentTokenBought,currentToken
        }).then(function () {
